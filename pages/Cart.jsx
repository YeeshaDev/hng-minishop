...
function Cart() {
  ...
  const {totalAmount,totalItems} = useSelector(
    (state) => state.cart)
  ...
  return (
    <div className='relative'>
              ...
              <article className="w-full md:w-1/2">
                {cart?.length > 0 &&
                  cart.map((product, index) => (
                   ...
                   <button className='flex items-center justify-end mt-5' >Clear Cart</button>
              </article>
              {/* Checkout */}
      ...
      <div className=" text-Clr">
        <h2 className="text-xl text-center my-7 font-bold">Order Summary</h2>
        <div className="flex flex-col gap-6 mt-5">
          <p className="flex justify-between gap-x-10">
            Order Value <span className="">${totalAmount.toFixed(2)}</span>
          </p>
          ...
          <p className="flex justify-between gap-x-10">
            Total before Discount <span>${(totalAmount + totalAmount * 0.15).toFixed(2)}</span>
          </p>
          ...
          >
            ...
            <button className='absolute top-1 right-4 flex items-center mt-3' >Clear Cart</button>
              {/** MOBILE CART */}
                    ...
                    <div className="flex-center gap-x-5">
                      ...
                      <div>
                        <div className=" ml-7">
                          ...
                          <p className="text-red-600 font-medium ">${product?.totalPrice.toFixed(2)}</p>
                          <button  onClick={() => handleRemove(product?.id)}  className="border-b-2 mt-1 border-textClr">
                            Remove
                          </button>
                        </div>
                        <div>
